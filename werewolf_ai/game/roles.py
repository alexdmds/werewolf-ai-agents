...
from agents.langchain_werewolf_agent import LangChainWerewolfAgent
...
# Nouvelle assignation avec LangChainWerewolfAgent
def assign_roles_langchain(nb_players=5):
    agents = []
    if nb_players < 3:
        raise ValueError("Il faut au moins 3 joueurs")
    def gen_id():
    ...
    seer_name = "Marie"
    # 1. Voyante
    agents.append(LangChainWerewolfAgent(seer_name, "Seer", agent_id=gen_id()))
    # 2. Loups (environ 1/3 des joueurs, arrondi sup)
    n_wolves = max(1, nb_players // 3)
    for i in range(n_wolves):
        agents.append(LangChainWerewolfAgent(wolf_names[i], "Werewolf", agent_id=gen_id()))
    # 3. Villageois (le reste)
    used_names = set([seer_name] + wolf_names[:n_wolves])
    v_names = [n for n in villager_names if n not in used_names]
    while len(agents) < nb_players:
        agents.append(LangChainWerewolfAgent(v_names.pop(0), "Villager", agent_id=gen_id()))
    return agents
